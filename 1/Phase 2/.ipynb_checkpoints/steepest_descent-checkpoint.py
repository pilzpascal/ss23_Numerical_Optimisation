...
def steepest_descent(
    ...
    """This function implements the steepest descent algorithm using exact line search.

    :param residual: gradient of the objective function
    :param x_0: starting point
    :param x_star: solution for the objective function
    ...
    while norm(r_k) > eps:
        ...
        alpha_k = (r_k.T @ r_k) / (r_k.T @ Q @ r_k)
        x_k = x_k - alpha_k * r_k
        r_k = residual(x_k)
        i += 1

    return x_k, np.array(norm(r_k)), np.array(norm(x_k - x_star)), np.array(i)
...
if __name__ == "__main__":
    ...
    ######## Problem iii) ########
    """
    print(steepest_descent(fc.l_f1_grad, np.zeros(shape=5), np.array([5., -120., 630., -1120.00000001, 630.]), 5))
    print(steepest_descent(fc.l_f2_grad, np.zeros(shape=8), np.array(
        [-8.00000052e+00, 5.04000025e+02, -7.56000030e+03, 4.62000015e+04, -1.38600004e+05, 2.16216005e+05,
         -1.68168004e+05, 5.14800010e+04]), 8))
    print(steepest_descent(fc.l_f3_grad, np.zeros(shape=12),
    ...
    print(steepest_descent(fc.l_f4_grad, np.zeros(shape=20),
    ...
    print(steepest_descent(fc.l_f5_grad, np.zeros(shape=30),
